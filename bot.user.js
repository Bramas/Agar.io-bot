...
function getLatestCommit() {
    window.jQuery.ajax({
            ...
            $.get('https://raw.githubusercontent.com/Bramas/Agar.io-bot/master/bot.user.js?' + Math.floor((Math.random() * 1000000) + 1), function(data) {
                ...
                {
                    update("aposBot", "bot.user.js", "https://github.com/Bramas/Agar.io-bot/blob/" + sha + "/bot.user.js/");
                }
                ...
                for (var k = 0; k < player.length; k++) { 
                    ...
                    for (var i = 0; i < allPossibleThreats.length; i++) {
                        ...
                        if (enemyCanSplit) {
                            ...
                            drawCircle(allPossibleThreats[i].x, allPossibleThreats[i].y, normalDangerDistance + allPossibleThreats[i].size, 3);
                        ...
                        else
                        {
                            if (enemyCanSplit && relativeDistance < 2*splitDangerDistance) {
                                ...
                                px /= enemyDistance;
                                py /= enemyDistance;
                                var factor = (2*splitDangerDistance - relativeDistance)/(2.0*splitDangerDistance);
                                factor *= factor;
                                factor *= 100;
                                px *= factor;
                                py *= factor;
                                ...
                                px /= enemyDistance;
                                py /= enemyDistance;
                                var factor = (2*normalDangerDistance - relativeDistance)/(2.0*normalDangerDistance);
                                factor *= factor;
                                factor *= 100;
                                px *= factor;
                                py *= factor;
